...
class ArgumentMining:
    ...
    def pre_processing(self, session, content_id, title, content, params_dict_str):
        op_logger.info(f'content_id: {content_id}')
...
if __name__ == "__main__":
    ...
    content_id = str.encode(body.get('itemId', ''))
    title = str.encode(body.get('title', ''))
    content = str.encode(body.get('content', ''))
    ...
    result = argument_mining.post_processing(session, result)
    print(result)
